...
class user_auth_services {
    async requestSignup(payload: signupPayload) {
        let user = await UserE.getUser({ email: payload.email })
        if (user)
            throw new Error("User already associated with following email")
        let phone = await UserE.getUser({ phone: payload.phone })
        if (phone)
    ...
    async verifyEmailandSignup(payload: signupVerification) {
        ...
        return await UserE.create(payload_data)
...
export const userAuthServices = new user_auth_services();
